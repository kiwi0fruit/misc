...
|        |           |               |                |
| **я**  | **ya**    | **ь**         | **\_j\_**      |
| `\cья` | `[\c]jya` | `[\c]ь[еёюя]` | `[\c]jy[eoua]` |
| `\cъя` | `[\c]ja`  | `[\c]ьи`      | `[\c]jyi`      |
| `ыя`   | `yja`     | `[\c]ь`       | `[\c]j`        |
|        |           |               |                |
| **щ**  | **shh**   |               |                |

`\c` - согласная (consonant) - спациальный символ регулярных выражений. Это будут солгасные латиницы`[qwrtpsdfghjklzxcvbnm]` и согласные кириллицы без `й`: `[цкнгшщзхфвпрлджчсмтб]`.

Простые замены второго шага (туда-обратно, lookbehind и lookahead нужны только при обратной замене):
